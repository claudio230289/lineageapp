...
// ACCIÓN EXPLÍCITA: GUARDAR EN LA NUBE (FIREBASE)
...
export function importarRespaldoJSONAuto(event) {
    ...
    reader.onload = async function(e) {
        try {
            ...
            renderizarTodo();
            if (document.getElementById('tab-anual')?.classList.contains('active')) renderizarGraficoAnual();
            alert('¡Respaldo importado y guardado en la nube con éxito!');
        } catch (err) {
...
export function accionGuardarJSON() {
    if (confirm("¿Deseás descargar la copia de respaldo local en este dispositivo?")) {
...
export function renderizarTodo() {
    try {
        ...
        // RENDERIZADO TABLA ANUAL SIN SALTOS Y CON COLORES
        ...
        if (tablaAnual && selectorAnio) {
            ...
            for (let m = 1; m <= 12; m++) {
                ...
                const colorSaldo = saldo < 0 ? 'text-rose-600' : 'text-emerald-600';

                const tr = document.createElement('tr');
                tr.className = 'border-b border-gray-100 hover:bg-gray-50/50 transition';
                tr.innerHTML = `
                    <td class="py-2.5 pr-2 font-medium text-gray-800 whitespace-nowrap">${mesesNombres[m-1]} ${baseYear}</td>
                    <td class="py-2.5 px-2 text-right font-mono font-bold text-emerald-600 whitespace-nowrap">${formatARS(net)}</td>
                    <td class="py-2.5 px-2 text-right font-mono font-bold text-rose-600 whitespace-nowrap">${formatARS(gasto.total)}</td>
                    <td class="py-2.5 pl-2 text-right font-mono font-bold ${colorSaldo} whitespace-nowrap">${formatARS(saldo)}</td>
                `;
